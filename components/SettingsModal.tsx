...
export const SettingsModal: React.FC<SettingsModalProps> = ({ 
  ...
  const handleNativeImport = async () => {
    ...
    // We pass existing IDs for duplicate detection
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto">
      <div className="bg-warm-beige dark:bg-slate-900 border border-sandstone dark:border-slate-700 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col my-8 relative overflow-hidden">
        ...
        <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto">
              ...
              <section>
                ...
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 rounded-full bg-sandstone/20 dark:bg-slate-800 border-2 border-dashed border-sandstone dark:border-slate-700 flex items-center justify-center overflow-hidden cursor-pointer hover:border-sage-green group relative" onClick={() => fileInputRef.current?.click()}>
                    {localSettings.userAvatar ? <img src={localSettings.userAvatar} className="w-full h-full object-cover" /> : <PlusIcon />}
          ...
          {activeTab === 'ai' && (
            <section className="space-y-6 text-left">
              <div>
                <label className="block text-[10px] font-bold text-moss-brown mb-4 uppercase tracking-widest">Provider</label>
              ...
              <div className="bg-sandstone/5 dark:bg-slate-800/30 p-6 rounded-2xl border border-sandstone/20 space-y-4">
                {localSettings.aiProvider === AIProvider.LMSTUDIO ? (
                  <input type="text" value={localSettings.customEndpoint} onChange={(e) => setLocalSettings({...localSettings, customEndpoint: e.target.value})} className="w-full bg-white dark:bg-slate-900 border border-sandstone rounded-lg px-4 py-2 text-sm" placeholder="Endpoint URL" />
                ) : localSettings.aiProvider !== AIProvider.GEMINI && (
                  <input type="password" value={localSettings.apiKeys[localSettings.aiProvider] || ''} onChange={(e) => updateApiKey(localSettings.aiProvider, e.target.value)} className="w-full bg-white dark:bg-slate-900 border border-sandstone rounded-lg px-4 py-2 text-sm" placeholder={`${localSettings.aiProvider} API Key`} />
                )}
                <button onClick={handleConnect} disabled={isFetchingModels} className="w-full py-2 bg-white dark:bg-slate-700 rounded-lg text-xs font-bold border border-sandstone flex items-center justify-center gap-2">
                  {isFetchingModels ? <RefreshIcon /> : 'Fetch Models'}
                </button>
                {fetchError && <p className="text-[10px] text-red-500 font-bold">{fetchError}</p>}
                <select value={localSettings.preferredModel} onChange={(e) => setLocalSettings({...localSettings, preferredModel: e.target.value})} className="w-full bg-white dark:bg-slate-900 border border-sandstone rounded-lg px-4 py-2 text-sm">
